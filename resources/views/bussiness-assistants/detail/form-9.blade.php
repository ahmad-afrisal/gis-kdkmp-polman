            ...
            <div class="py-8">
                <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <nav class="flex items-center text-sm text-gray-600 space-x-2 mb-7">
                        ...
                        <span>›</span>
                        <span class="text-gray-500">Form 3 2026</span>
                    </nav>
                    @if (session('error'))
                    ...
                    <div class="bg-white shadow-xl rounded-lg p-6">
                        ...
                        <form action="{{ route('bussiness-assistants.form-9.store', $bussinessAssistant->id) }}"
                            method="POST">
                            @csrf
                                ...
                                <table class="min-w-full text-sm text-center border-collapse">
                                    ...
                                    <thead>
                                        <tr class="bg-green-600 ">
                                            ...
                                            <th rowspan="2"
                                                class="sticky left-[45px] z-20 bg-green-600 w-[800px] min-w-[300px] border border-gray-300 px-2 py-1">
                                                Nama KDKMP
                                            </th>

                                            <th colspan="3" class="border border-gray-300 px-3 py-2">Status Gerai
                                            </th>
                                            <th colspan="2"
                                                class="w-[600px] min-w-[300px border border-gray-300 px-3 py-2">Jumlah
                                                Karyawan
                                                / Pengelola</th>
                                            <th colspan="2" class="border border-gray-300 px-3 py-2">Panduan
                                                Operasional Gerai
                                            </th>
                                            <th rowspan="2"
                                        ...
                                        <tr class="bg-green-700 ">
                                            <th class="border border-gray-300 px-3 py-2">Belum Ada</th>
                                            <th class="border border-gray-300 px-3 py-2">Belum Buka</th>
                                            <th class="border border-gray-300 px-3 py-2">Belum Operasional</th>

                                            <th class="w-[150px] min-w-[80px] border border-gray-300 px-3 py-2">Jumlah
                                                2025</th>
                                            <th class="w-[150px] min-w-[80px] border border-gray-300 px-3 py-2">Jumlah
                                                2026</th>
                                    ...
                                    <tbody class="text-gray-700">
                                        @forelse ($cooperations as $index => $coop)
                                            @php
                                                $form = $coop->formNine; // Bisa null, aman
                                            @endphp
                                            <tr class="odd:bg-gray-50 even:bg-white hover:bg-green-50 transition">
                                                ...
                                                <td class="border border-gray-300 px-2 py-1 text-center">
                                                    <input type="checkbox" class="checkbox-status"
                                                        data-group="outlet_status_{{ $index }}" data-type="belum"
                                                        name="data[{{ $index }}][outlet_status_belum]"
                                                        value="0"
                                                        {{ $form?->outlet_status == 0 ? 'checked' : '' }}>
                                                </td>
                                                <td class="border border-gray-300 px-2 py-1 text-center">
                                                    <input type="checkbox" class="checkbox-status"
                                                        data-group="outlet_status_{{ $index }}" data-type="ada"
                                                        name="data[{{ $index }}][outlet_status_ada]"
                                                        value="1"
                                                        {{ $form?->outlet_status == 1 ? 'checked' : '' }}>
                                                </td>
                                                <td class="border border-gray-300 px-2 py-1 text-center">
                                                    <input type="checkbox" class="checkbox-status"
                                                        data-group="outlet_status_{{ $index }}"
                                                        data-type="tidak"
                                                        name="data[{{ $index }}][outlet_status_tidak]"
                                                        value="2"
                                                        {{ $form?->outlet_status == 2 ? 'checked' : '' }}>
                                                </td>
                                                <td class="border border-gray-300 px-2 py-1">
                                                    <input type="number"
                                                        name="data[{{ $index }}][number_of_employees_2025]"
                                                        value="{{ $form?->number_of_employees_2025 ?? '' }}"
                                                        class="w-full border-gray-300 rounded-lg text-sm p-2 focus:ring-2 focus:ring-green-400 focus:outline-none">
                                                </td>

                                                <td class="border border-gray-300 px-2 py-1">
                                                    <input type="number"
                                                        name="data[{{ $index }}][number_of_employees_2026]"
                                                        value="{{ $form?->number_of_employees_2026 ?? '' }}"
                                                        class="w-full border-gray-300 rounded-lg text-sm p-2 focus:ring-2 focus:ring-green-400 focus:outline-none">
                                                </td>
                                                ...
                                                <td class="border border-gray-300 px-2 py-1 text-center">
                                                    <input type="checkbox" class="checkbox-status"
                                                        data-group="outlet_operations_guide_{{ $index }}"
                                                        data-type="yes"
                                                        name="data[{{ $index }}][outlet_operations_guide]"
                                                        value="1"
                                                        {{ $form?->outlet_operations_guide === 1 ? 'checked' : '' }}>
                                                </td>
                                                <td class="border border-gray-300 px-2 py-1 text-center">
                                                    <input type="checkbox" class="checkbox-status"
                                                        data-group="outlet_operations_guide_{{ $index }}"
                                                        ...
                                                        name="data[{{ $index }}][outlet_operations_guide]"
                                                        value="0"
                                                        {{ $form?->outlet_operations_guide === 0 ? 'checked' : '' }}>
                                                </td>
